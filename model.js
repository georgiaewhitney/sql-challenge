...
const select_students_in_finsbo = db.prepare(/*sql*/ `
  SELECT username FROM students WHERE cohort_name IN (SELECT name FROM cohorts WHERE location = 'Finsbury Park')
`);
...
const select_students_with_location = db.prepare(/*sql*/ `
  ...
  FROM students JOIN cohorts ON students.cohort_name = cohorts.name
`);
...
const select_students_with_projects = db.prepare(/*sql*/ `
  ...
  FROM projects JOIN students_projects ON projects.id = students_projects.project_id JOIN students ON students.username = students_projects.student_username  
`);
...
const select_students_with_projects_in_finsbo = db.prepare(/*sql*/ `
  SELECT projects.name, students.username FROM projects JOIN students_projects ON projects.id = students_projects.project_id JOIN students ON students.username = students_projects.student_username WHERE students.username IN ( SELECT username FROM students WHERE cohort_name IN ( SELECT name FROM cohorts WHERE location = 'Finsbury Park')) 
`);
